let libraryPlaylists = new Map(); // playlists that can be auto-generated from the library
let userPlaylists = new Map();    // additional custom user playlists
 ...
 */
 async function createUserPlaylist() {
  ...
  if (playlistName.length !== 0) {
    ...
    const option = `<option id="playlist-option-${playlistName}" value="${playlistName}" selected>${playlistName}</option>`;
